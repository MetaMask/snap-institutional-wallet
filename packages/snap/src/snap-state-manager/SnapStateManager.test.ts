...
import * as snapUtil from './snap-util';
import { SnapStateManager } from './SnapStateManager';
...
describe('SnapStateManager', () => {
  ...
  const createMockState = (initState: MockState) => {
    const setStateDataFn = async ({
      ...
      ).reduce((acc, [key, value]: [string, MockTransactionDetail]) => {
        (acc as Record<string, MockTransactionDetail>)[key] = {
          ...value,
        };
        return acc;
      }, {});
    ...
    const updateDataFn = (
      ...
      if (
        !Object.prototype.hasOwnProperty.call(state.transactionDetails, data.id)
      ) {
        ...
        state.transactionDetails[data.id] = {
          txHash: data.txHash,
          cnt: (state.transactionDetails[data.id]?.cnt ?? 0) + data.cnt,
        };
      }
    };
    ...
    const setStateDataSpy = jest
      ...
      .mockImplementation(async ({ data, encrypted }) => {
        await setStateDataFn({ data: data as MockState, encrypted });
      });
  ...
  describe('withTransaction', () => {
    ...
    it('does not rollback if rollback failed and has committed', async () => {
      ...
      setStateDataSpy
        .mockImplementationOnce(
          async (args: { data: unknown; encrypted: boolean }) =>
            setStateDataFn(args as { data: MockState; encrypted: boolean }),
        )
        .mockImplementationOnce(() => {
    ...
    it('throws an Error if another Error was thrown', async () => {
      ...
      setStateDataSpy
        ...
        .mockImplementationOnce(setStateDataFn as any);
